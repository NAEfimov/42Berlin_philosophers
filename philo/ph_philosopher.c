...
/*   Updated: 2025/06/02 12:49:05 by nefimov          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ph_philo_init(t_philo *philo, t_mtxs *mtxs, int *args, int i)
{
	...
	if (i > 0)
	{
		philo->frks[0] = &mtxs->frks[i - 1];
		philo->frks[1] = &mtxs->frks[i];
		philo->frk_mtx[0] = &mtxs->frk_mtx[i - 1];
		philo->frk_mtx[1] = &mtxs->frk_mtx[i];
	}
	else
	{
		philo->frks[0] = &mtxs->frks[i];
		philo->frks[1] = &mtxs->frks[philo->ph_num - 1];
		philo->frk_mtx[0] = &mtxs->frk_mtx[i];
		philo->frk_mtx[1] = &mtxs->frk_mtx[philo->ph_num - 1];
	}
